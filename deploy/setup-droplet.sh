...
echo ""
echo " Next steps:"
echo "  1. Note your BOOTSTRAP_ADMIN_KEY (used to create the first admin/manager account):"
echo "       ${BOOTSTRAP_KEY_GENERATED}"
echo "  2. Configure SMTP for the contact form:"
echo "       nano ${APP_DIR}/.env"
echo "     Set: SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS, CONTACT_TO"
echo "  3. Restart the app after editing .env:"
echo "       pm2 restart building-company"
if [[ -n "${DOMAIN}" ]]; then
  echo "  4. Obtain an SSL certificate:"
  echo "       sudo certbot --nginx -d ${DOMAIN} -d www.${DOMAIN}"
else
  echo "  4. Set your domain in /etc/nginx/sites-available/building-company"
  echo "     then run: sudo certbot --nginx -d YOUR_DOMAIN"
fi
echo "  5. Create your first admin account (use the BOOTSTRAP_ADMIN_KEY above):"
echo "       See README – section 'Krok 8: Utwórz konto administratora'"
echo "════════════════════════════════════════════════"
