...
const DEFAULT_PATHS = {
  create: '/',
  preview: '/:publicToken',
  attachments: '/:publicToken/attachments',
  claimRequest: '/:id/claim/request',
  claimConfirm: '/:id/claim/confirm'
};
...
const registerPublicGuestQuoteRoutes = (router, { auth, paths = DEFAULT_PATHS } = {}) => {
  if (!router) {
    throw new Error('registerPublicGuestQuoteRoutes requires an express router instance.');
  }

  if (typeof auth !== 'function') {
    throw new Error('registerPublicGuestQuoteRoutes requires an auth middleware.');
  }

  const routePaths = { ...DEFAULT_PATHS, ...(paths || {}) };
  ...
  router.get(routePaths.preview, [param('publicToken').isLength({ min: 16 })], asyncHandler(async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({ errors: errors.array() });
    }
...
module.exports = {
  buildGuestQuotePreviewPayload,
  loadGuestQuotePreviewRecord,
  registerPublicGuestQuoteRoutes
};
