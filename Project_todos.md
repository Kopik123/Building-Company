...
- [ ] Continue the `web-v2` refactor after the new route-module/config split by extracting shared domain hooks and breaking the grouped workspace pages into smaller domain-owned modules.
...
- [ ] Continue the `web-v2` split beyond grouped route modules by breaking `projects+quotes`, `messaging` and `operations` into smaller domain files/hooks after this routing/config wave settles.
