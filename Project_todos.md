...
- [ ] Investigate the `web-v2` client quote follow-up upload timeout in `tests/playwright/web-v2-rollout.spec.js`; after the route-module split the suite stayed mostly green (`12/15`), but the `Upload more photos` step still timed out in desktop/mobile browsers.
