...
- [ ] Validate the DigitalOcean deployment after the build-chain fix — confirm the buildpack runs `npm ci --omit=dev && npm run build`, the nested `apps/web-v2` install keeps Vite build dependencies, the root `/dist` assets upload, and the Node process starts on port 8080.
