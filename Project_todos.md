...
- [ ] Continue dependency cleanup after the `bcrypt` upgrade: the remaining production audit warnings now come from `umzug -> @rushstack/ts-command-line -> ajv`, so decide whether to upgrade `umzug` next or accept that moderate-only chain for now.
