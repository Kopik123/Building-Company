...
- [ ] Decide whether local migration tooling should go further than the new preflight message and support a safe dev bootstrap path (for example a documented local Postgres/Compose setup) instead of requiring manual `DATABASE_URL` export every time.
