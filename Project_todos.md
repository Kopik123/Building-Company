...
- [ ] Continue the `web-v2` refactor by extracting smaller list/detail/editor modules and typed view-model adapters from `crm`, `private-inbox` and `inventory`, then revisit whether `quotes` also needs view-model adapters on top of the new domain folder split.
